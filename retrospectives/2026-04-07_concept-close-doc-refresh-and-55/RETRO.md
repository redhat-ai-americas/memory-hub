...
| **Three pre-existing untracked docs directories** (`docs/auth/`, `docs/identity-model/`, `docs/scenarios/`) have been in git status since session start and stayed untracked the whole session. They're presumably work-in-progress from other sessions. Clutter every `git status` output. | **Process gap** — should have asked what they are. Either they need to be tracked (they're real work) or gitignored (they're local drafts). Living in the untracked limbo is noise. Not filing as an issue; defer to next session. |
...
- [ ] Decide what to do with `docs/auth/`, `docs/identity-model/`, `docs/scenarios/` (track or gitignore).
