...
# -- update_memory edge re-pointing (#472) --


async def test_update_memory_repoints_outgoing_edges(async_session, embedding_service):
    """After update, outgoing edges from the updated memory point to the new version."""
    a, _ = await create_memory(_make_create_data(content="memory A"), async_session, embedding_service)
    b, _ = await create_memory(_make_create_data(content="memory B"), async_session, embedding_service)

    edge = MemoryRelationship(
        id=uuid.uuid4(),
        source_id=a.id, target_id=b.id,
        relationship_type="related_to",
        created_by="test", tenant_id="default",
    )
    async_session.add(edge)
    await async_session.commit()

    updated_a = await update_memory(a.id, MemoryNodeUpdate(content="memory A v2"), async_session, embedding_service)
...
async def test_update_memory_repoints_incoming_edges(async_session, embedding_service):
...
async def test_update_memory_preserves_invalidated_edges(async_session, embedding_service):
    """Invalidated edges (valid_until set) are not re-pointed on update."""
    a, _ = await create_memory(_make_create_data(content="memory A"), async_session, embedding_service)
    b, _ = await create_memory(_make_create_data(content="memory B"), async_session, embedding_service)
    old_a_id = a.id
    ...
    await update_memory(a.id, MemoryNodeUpdate(content="memory A v2"), async_session, embedding_service)
