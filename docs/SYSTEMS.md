...
| mcp-server | MCP interface with 10 tools: register_session, search_memory, read_memory (with paginated version history), write_memory (with project auto-enrollment), update_memory, delete_memory, manage_project, manage_session (status/set_focus/focus_history), manage_graph (create_relationship/get_relationships/get_similar), manage_curation (report_contradiction/set_rule). Tool consolidation (#173/#174) merged suggest_merge into manage_graph and get_memory_history into read_memory; #188 added manage_project; session/graph/curation tools further consolidated | [mcp-server.md](mcp-server.md) | Implemented |
