...
| mcp-server | MCP interface with 14 tools: register_session, search_memory, read_memory (with paginated version history), write_memory (with project auto-enrollment), update_memory, delete_memory, list_projects, get_similar_memories, get_relationships, create_relationship (with merge-suggestion pattern), set_curation_rule, report_contradiction, set_session_focus, get_focus_history. Tool consolidation (#173/#174) merged suggest_merge into create_relationship and get_memory_history into read_memory; #188 added list_projects | [mcp-server.md](mcp-server.md) | Implemented |
