...
| mcp-server | MCP interface with 13 tools: register_session, search_memory, read_memory (with paginated version history), write_memory, update_memory, delete_memory, get_similar_memories, get_relationships, create_relationship (with merge-suggestion pattern), set_curation_rule, report_contradiction, set_session_focus, get_focus_history. Tool consolidation (#173/#174) merged suggest_merge into create_relationship and get_memory_history into read_memory | [mcp-server.md](mcp-server.md) | Implemented |
