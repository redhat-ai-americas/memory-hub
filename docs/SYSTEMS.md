...
|-----------|-------------|-----|--------|
| memory-tree | Core data model: tree-structured memories with nodes, branches, weights, and scopes. Content type system (#237, #552) classifies memories as experiential/knowledge/behavioral/procedural for graduation, reconstruction, and directed-graph runbooks. Workflow checkpoint (#238) provides durable state for recurring agents | [memory-tree.md](design/memory-tree.md) | Implemented |
