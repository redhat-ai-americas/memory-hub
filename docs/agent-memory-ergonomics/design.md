...
**Status: All three layers shipped 2026-04-07.** Layer 1 (search shape, #56/#57) and Layer 3 (project config + CLI, #59/#60/#73) shipped earlier in the day. Layer 2 (session focus vector with two-vector retrieval, #58) shipped at the end of the day -- a cross-encoder reranker (`ms-marco-MiniLM-L12-v2`) was deployed mid-session and the design space pivoted to NEW-1 (RRF blend over cross-encoder rerank). Per-candidate status markers live in the [Implementation Candidates](#implementation-candidates) section.
