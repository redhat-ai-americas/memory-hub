...
That forensic chain is not just good engineering practice; it maps onto emerging regulation. The EU AI Act (enforcement begins August 2, 2026) requires automatic logging of AI system behavior (Article 12), traceability of decisions (Article 19), and data minimization. GDPR adds the right to erasure (Article 17) and the right to explanation (Article 22). MemoryHub's governance substrate -- versioned memories, scoped access, cascade-aware deletion, and graph-traversable provenance -- supports these requirements out of the box rather than as a bolt-on. For regulated industries (government, financial services, healthcare, defense), this is the difference between an agent memory system you can deploy and one you cannot.

Start with the [agent integration guide](agent-integration-guide.md) to wire up a harness, and [ARCHITECTURE.md](../ARCHITECTURE.md) for the system view. To go deeper on the *dimensions* of agent memory — the classification axes (temporal scope, cognitive type, storage strategy, architectural role) with full literature citations — read [Agent Memory Foundations](https://github.com/redhat-ai-americas/memory-hub-scratchpad/blob/main/research/agent-memory-foundations.md); for the graph-memory landscape, benchmarks and their caveats, and the compiled-knowledge (llm-wiki) ecosystem, see the rest of [research/](https://github.com/redhat-ai-americas/memory-hub-scratchpad/tree/main/research).
