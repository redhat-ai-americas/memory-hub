...
Dialect-portable version: uses PortableUUID and JsonEncodedList instead of
PostgreSQL-specific UUID, ARRAY, and partial indexes.
"""
...
from memoryhub_local.models.dialect import JsonEncodedList, PortableUUID
...
class ConversationThread(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        Index("ix_conv_threads_scope_id", "scope_id"),
        Index("ix_conv_threads_a2a_context_id", "a2a_context_id"),
        Index("ix_conv_threads_status", "status"),
        Index("ix_conv_threads_deleted_at", "deleted_at"),
        Index("ix_conv_threads_expires_at", "expires_at"),
    )
...
class ConversationMessage(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_conv_messages_actor_id", "actor_id"),
        Index("ix_conv_messages_tool_call_id", "tool_call_id"),
    )
...
class PurgeLog(Base):
    ...
    resource_id: Mapped[uuid.UUID] = mapped_column(PortableUUID(), nullable=False)
